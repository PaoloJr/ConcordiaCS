...
int main() {
    ...
    test("Constructor adds line number", token1.getLineNumbers().size() == 1);
    test("Constructor stores correct line number", token1.getLineNumbers().at(0) == 5);

    // Test constructor with string object and line number
    ...
    test("Constructor adds line number", token2.getLineNumbers().size() == 1);
    test("Constructor stores correct line number", token2.getLineNumbers().at(0) == 10);

    // Test appendLineNumber method
    cout << "\n[Testing appendLineNumber Method]" << endl;
    token1.appendLineNumber(15);
    test("appendLineNumber adds to list", token1.getLineNumbers().size() == 2);
    test("appendLineNumber preserves first line", token1.getLineNumbers().at(0) == 5);
    test("appendLineNumber adds correct line", token1.getLineNumbers().at(1) == 15);
    
    // Add more line numbers
    token1.appendLineNumber(25);
    token1.appendLineNumber(35);
    test("appendLineNumber works for multiple additions", token1.getLineNumbers().size() == 4);
    test("Line numbers are stored correctly", 
         token1.getLineNumbers().at(0) == 5 &&
         token1.getLineNumbers().at(3) == 35);

    // Test getToken method
    cout << "\n[Testing getToken Method]" << endl;
    const string& retrievedToken = token1.getToken();
    ...
    const vector<int>& lines = token1.getLineNumbers();
    test("getLineNumbers returns correct size", lines.size() == 4);
